package com.example.reverseproxy.util;
...
public class DesEncryptUtil {
     ...
     * * 默认转换模式transformation为：DES/ECB/PKCS5Padding * * @param toEncryptData *
     * 需要加密的数据 * @param keyData * 密钥数据内容, 使用该数据的byte[]缓冲区前 8 个字节作为密钥 * @return
     * 返回加密后的数据，以16进制字符串形式表示 @ * * @creator TRS
     */
    ...
    /**
     * * 获取Cipher加密和解密对象 * * @param key * 密钥 * @param cipherMode }
     *
     *
     *
     * 此 Cipher的操作模式（为以下之一：ENCRYPT_MODE、 DECRYPT_MODE、WRAP_MODE 或 UNWRAP_MODE）
     * * @param transformation * 转换的名称，例如 DES/CBC/PKCS5Padding。有关标准转换名称的信 息，请参见 Java
     * Cryptography Architecture Reference Guide 的附录 A。 * @return * @creator TRS
     */
    ...
    /**
     * * 根据指定密钥，对DES加密数据进行解密 * * @pram encryptedDataHex 加密字符串(十六进制) * @param keyData
     * * 密钥 * @return 解密字符串 * @throws Throwable * * @creator TRS
     */
     ...
     * * 默认转换模式transformation为：DES/ECB/PKCS5Padding * * @param toDecryptData *
     * 要解密的数据 * @param keyData * DES密钥数据，使用该数据的byte[]缓冲区前 8 个字节作为密钥 * @return
     * 返回解密后的数据，以16进制字符串形式表示 @ * * @creator TRS
     */
    ...
    /**
     * * 指定密钥和转换方式，进行数据解密 * * @param key * 密钥 * @param toDecryptData * 要解密的数据
     * * @param transformation * * 转换的名称，例如 DES/CBC/PKCS5Padding。有关标准转换名称的 信息，请参见
     * Java Cryptography Architecture Reference Guide 的附录 A。 * @return 解密后的数据
     * * @since * @creator TRS
     */
    ...
    /**
     * * 根据密钥数据生成DES密钥 * * @param keyData * DES密钥数据，使用该数据的byte[]缓冲区前 8 个字节作为密钥
     * * @return DES密钥 * * @creator TRS
     */
     ...
     *             当给定十六进制字符串的长度为奇数时或给定字符串包含非十六进 制字符.
     * @see #toString(byte[])
     */
